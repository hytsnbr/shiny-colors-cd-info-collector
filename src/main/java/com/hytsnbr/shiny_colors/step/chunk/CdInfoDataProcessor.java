...
public class CdInfoDataProcessor implements ItemProcessor<DiscographyData, CdInfo> {
    ...
    /** 発売日取得時の日付フォーマット */
    private static final DateTimeFormatter releaseDateTimeFormatter =
    ...
    private List<String> getRecordNumbers(TextNode node) throws CdInfoWebScrapingException {
        ...
        var recordNumberText = node.toString().replaceAll("[\r\n]", "").trim().replace("品番：", "");
     ...
     */
    private String getNormalizationStoreSiteUrl(String siteUrl, Store store)
            throws CdInfoWebScrapingException {
        URL url;
        try {
